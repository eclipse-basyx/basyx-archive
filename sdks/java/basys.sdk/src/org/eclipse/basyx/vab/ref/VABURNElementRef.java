package org.eclipse.basyx.vab.ref;
...
/**
 * URN reference to a remote property. This reference also contains RTTI information
 * 
...
public class VABURNElementRef extends VABElementRef implements IElementReference {
	...
	/**
	 * Store element path part of reference. This path is the logic path to the property that a user will use.
	 */
	...
	/**
	 * Store server path to element. This path may contain optimized handles and is used internally by the SDK.
	 */
	protected String serverElementPath = null;
	...
	public VABURNElementRef(IElement element) {
		...
		// - AAS
		if (element instanceof IAssetAdministrationShell) {legalBody = element.getId();} 
		// - Sub model
		else if (element instanceof ISubModel) {legalBody = ElementTools.getAASID(element); subUnit = element.getId();} 
		// - Sub model property, event or operation
		// - FIXME: Interface types
		else if (element instanceof IProperty) {legalBody = ElementTools.getAASID(element); subUnit = ElementTools.getSubmodelID(element); elementPath = ElementTools.getFullPathToProperty(element);}
		else if (element instanceof Operation) {legalBody = ElementTools.getAASID(element); subUnit = ElementTools.getSubmodelID(element); elementPath = ElementTools.getFullPathToProperty(element);}
		else if (element instanceof Event)     {legalBody = ElementTools.getAASID(element); subUnit = ElementTools.getSubmodelID(element); elementPath = ElementTools.getFullPathToProperty(element);}
		else {
		...
		if (serverElementPath == null) serverElementPath=elementPath;
	}
	...
	public boolean isAASReference() {
		...
		if ((legalBody != null) && ((subUnit == null) || (subUnit.length() == 0)) && ((elementPath == null) || (elementPath.length() == 0))) return true;
	...
	public boolean isSubModelReference() {
		...
		if ((subUnit != null) && ((elementPath == null) || (elementPath.length() == 0))) return true;
	...
	public boolean isPropertyReference() {
		...
		if ((subUnit != null) && (elementPath != null)) return true;
	...
	public String getId() {
		...
		if ((subUnit == null) || (subUnit.length()==0)) return legalBody;
		
		// If path is null or empty, ID is sub model ID
		if ((elementPath == null) || (elementPath.length()==0)) return subUnit;
	...
	public void addScope(String scope) {
		...
		if (legalBody == null)       {legalBody = scope; return;}
		if (legalBody.length() == 0) {legalBody = scope; return;}

		// Check if scope is already added
		if (legalBody.endsWith(scope)) return;
