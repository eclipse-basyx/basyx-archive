 ...
 * 
 * <p>The string must be formatted according to the 
 * <a href="https://reference.opcfoundation.org/v104/Core/docs/Part4/A.2/">BNF format of relative paths</a>. 
 * 
 * <p>Note that this helper doesn't currently support browse paths which directly specify a 
 * reference type to follow, using the <code>&lt;...&gt;</code> syntax. Only the more general hierarchical
...
public final class BrowsePathHelper {
	...
	private QualifiedName parseQualifiedName() {
		...
		if (s.length == 1) {
			...
			throw new OpcUaException(String.format("Not a valid relative path element starting at index %d: %s", index, path));
		}
	...
	private QualifiedName parseQualifiedName(String browseName) {
		...
		if (unescaped.isEmpty()) {
			throw new OpcUaException(String.format("Browse path contains invalid browse name at index %d: %s", index, path));
		}
	...
	private QualifiedName parseQualifiedName(String namespaceIndex, String browseName) {
		...
		try {
			...
			throw new OpcUaException(String.format("Browse path contains invalid namespace index at index %d: %s", index, path));
		}
	 ...
	 * @throws UnsupportedOperationException if the browse path contains directly specified
	 *  									 reference types using the <code>&lt;...&gt;</code> syntax.
	 ...
	 * 
	 * @throws IllegalArgumentException if <code>browsePath</code> is <code>null</code> or it's 
	 ...
	 * <a href="https://reference.opcfoundation.org/v104/Core/docs/Part4/A.2/">BNF format of relative paths</a>.
	 * 
	 * @throws IllegalArgumentException if relativePath contains references, which don't map to 
	 * 								    either '/' or '.' or if it is invalid.
	 */
