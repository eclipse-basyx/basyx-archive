 ...
 * 
 * <p>When this class is instantiated with an endpoint URL, an {@link IOpcUaClient} is
 * automatically created with a default configuration. The default is an insecure, anonymous
 * connection without a client certificate.
 * 
 * <p>The configuration can be changed as long as no connection has been established. A connection
 * is established automatically the first time a node is read or written or an operation is invoked,
 * either through this connector or directly through the associated {@link IOpcUaClient}.
 * <br>To change the configuration, do the following:
 * 
 * <pre>
 ...
 *   than one node. This would lead to an exception being returned from this connector.
 *   <br>It is up to the user to make sure their server's address space doesn't have that problem.
 ...
 * 
 * <p>If it is enabled, it will cache any browse path passed to {@link #getValue(String)},
 ...
 * 
 * <p><b>Caution:</b> An OPC UA server can dynamically reconfigure their address space
 * during runtime. This could even be done remotely from clients, if the server allows it.
 * <br>Such changes would render this cache invalid, but there is no way for this connector to get
...
public class OpcUaConnector implements IModelProvider {
	 ...
	 * 
	 * <p>When a {@link OpcUaConnector} resolves a browse path to a NodeId, it can cache the 
	 * result to make future access to that same browse path more efficient.
	 * <br>This setting controls how long looked up NodeIds remain cached. A value of 
	 * {@link Duration#ZERO} disables the cache.
	 * 
	 * <p>See {@link OpcUaConnector} for more information on the caching feature.
	 * 
