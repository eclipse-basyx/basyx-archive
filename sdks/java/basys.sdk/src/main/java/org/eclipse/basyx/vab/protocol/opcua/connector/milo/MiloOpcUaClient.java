...
public class MiloOpcUaClient implements IOpcUaClient {
	 ...
	 * 
	 * <p>The client will attempt to discover available endpoint at this URL. The one to use will
	 * be selected by looking at the security policy and message security mode. These can be 
	 * configured using {@link #setConfiguration(ClientConfiguration)}.
	 * 
	 * @param endpointUrl The client will attempt to discover available endpoints at this URL. Among
	 * 					  all of these the one that matches the configured security policy and 
	 * 					  message security mode will be selected. 
	 * 
	 ...
	 * 
	 * <p>See the documentation of {@link IOpcUaClient#getConfiguration()} for
	 * more information.
	 * 
	 ...
	 * 
	 * <p>See the documentation of {@link IOpcUaClient#setConfiguration(ClientConfiguration)} for
	 * more information.
	 */
	@Override
	 ...
	 * 
	 * <p>This method allows setting a Milo-specific configuration object which gives access to the
	 * full range of options that Milo supports as opposed to the limited selection in 
	 * {@link ClientConfiguration}.
	 * 
	 * <p>Be aware, that the <i>security policy</i> and <i>message security mode</i> for endpoint
	 * selection can only be set using {@link #setConfiguration(ClientConfiguration)}.
	 * <br>Additionally, for settings which are available in both configuration objects, 
	 * {@link ClientConfiguration} will take precedence over {@link OpcUaClientConfigBuilder}.
	 * 
	 * <p>The order in which you call {@link #setConfiguration(ClientConfiguration)} and 
	 ...
	 * 
	 * <p>See the documentation of {@link IOpcUaClient#hasConnected()} for
	 * more information.
	 */
	@Override
	 ...
	 * 
	 * <p>See the documentation of {@link IOpcUaClient#translateBrowsePathToNodeId(String)} for
	 * more information.
	 */
	@Override
	 ...
	 * Gets the id of the node matching the given browse path when resolved against the root node.
	 * 
	 * <p>See the documentation of {@link IOpcUaClient#translateBrowsePathToNodeIdAsync(String)} for
	 * more information.
	 */
	@Override
	 ...
	 * {@link IOpcUaClient#translateBrowsePathToNodeId(NodeId, String)} for more information.
	 */
	@Override
	 ...
	 * 
	 * <p>See the documentation of 
	 * {@link IOpcUaClient#translateBrowsePathToParentAndTargetNodeId(String)} for more information.
	 */
	@Override
	 ...
	 * 
	 * <p>See the documentation of {@link IOpcUaClient#readValue(NodeId)} for
	 * more information.
	 */
	@Override
	 ...
	 * Reads the current value from an OPC UA node.
	 * 
	 * <p>See the documentation of {@link IOpcUaClient#readValueAsync(NodeId)} for
	 * more information.
	 */
	@Override
	 ...
	 * 
	 * <p>See the documentation of {@link IOpcUaClient#writeValue(NodeId, Object)} for
	 * more information.
	 */
	@Override
	 ...
	 * Writes the value of an OPC UA node.
	 * 
	 * <p>See the documentation of {@link IOpcUaClient#writeValueAsync(NodeId, Object)} for
	 * more information.
	 */
	@Override
	 ...
	 * 
	 * <p>See the documentation of {@link IOpcUaClient#invokeMethod(NodeId, NodeId, Object...)} for
	 * more information.
	 */
	@Override
	 ...
	 * Invokes an OPC UA method on an object.
	 * 
	 * <p>See the documentation of {@link IOpcUaClient#invokeMethodAsync(NodeId, NodeId, Object...)}
		...
		return getClient()
				...
				.thenApply(result -> {
					if (!result.getStatusCode().isGood()) {
						throw new OpcUaException("Method invocation failed with status code: " + result.getStatusCode());
					}
	 ...
	 * 
	 * <p>If the client hasn't been created, yet, it will be created automatically, making any
	 * subsequent changes to the configuration impossible.
	 * 
	 * <b>Caution:</b> Use this method at your own risk. The underlying client API	might change
	 * without notice, making any user code relying on this method fragile.
	 * 
	...
	private OpcUaClientConfig buildMiloConfiguration(EndpointDescription endpoint) {
		OpcUaClientConfigBuilder builder = (this.miloConfiguration != null) ? this.miloConfiguration : createDefaultMiloConfigBuilder();
		builder.setApplicationName(LocalizedText.english(this.configuration.getApplicationName()))
				.setApplicationUri(this.configuration.getApplicationUri())
				.setCertificate(this.configuration.getCertificate())
				.setKeyPair(this.configuration.getKeyPair())
	...
	private UaClient createClient() {
		...
		Predicate<EndpointDescription> endpointFilter = ep -> { 
			boolean securityPolicyMatches = ep.getSecurityPolicyUri().equals(mapSecurityPolicy(securityPolicy).getUri());
	...
	/**
	 * Returns the first endpoint matching the filter predicate discovered at the
	 * endpointUrl.
	 * 
	 * @param endpointUrl The discovery endpoint to query.
	 * @param filter      A predicate that returns 'true' for any endpoint that can
	 *                    be used.
	 * 
	 * @return The first endpoint at the URL which matches the filter.
	 * 
	 * @throws OpcUaException if no endpoint matching the filter can be found at 
	 *						  the url or if the discovery thread is interrupted.
	 */
	 ...
	 * @return A future for a list of node ids. Ids are ordered in the same way as their matching 
	 * 		   browse paths.
	 * 
	 * @throws ResourceNotFoundException if any of the browse paths doesn't lead to any node at all.
	 * @throws AmbiguousBrowsePathException if any of the browse paths is ambiguous, i.e., it leads
	 * 										to multiple nodes.
	 * @throws OpcUaException if any other OPC UA related error occurs. This is a generic wrapper type for
	 ...
	 * 
	 * <p>While wrapping, value types for which equivalent types exist in Milo's type system
	 ...
	 * 
	 * <p>While unwrapping, data values with Milo-specific types will be converted to their 
	 * respective BaSyx equivalents.
	 * 
