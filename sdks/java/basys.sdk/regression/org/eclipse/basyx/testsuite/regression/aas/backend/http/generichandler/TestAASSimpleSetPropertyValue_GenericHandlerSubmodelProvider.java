 ...
 * - Retrieve AAS "Stub3AAS"
 * - Read and change properties
 * 
 * @author kuhn
 *
 */
class TestAASSimpleSetPropertyValue_GenericHandlerSubmodelProvider {
	...
	void runTest() throws Exception {
		...
		IAssetAdministrationShell connAAS2 = this.aasManager2.retrieveAAS("Stub3AAS");
		...
		ISubModel submodel2 = connAAS2.getSubModels().get("Stub3SM");		
		...
		((ISingleProperty) submodel.getProperties().get("testProp1")).set(5);
		((ISingleProperty) submodel.getProperties().get("testProp2")).set(6);
		...
		((ISingleProperty) submodel2.getProperties().get("testProp1")).set(2);
		((ISingleProperty) submodel2.getProperties().get("testProp2")).set(3);
		
		// Invalidate information of first connection
		((ConnectedElement) submodel.getProperties().get("testProp1")).invalidate();
		((ConnectedElement) submodel.getProperties().get("testProp2")).invalidate();
		
		// - Get property values (shortcut again)
		property1Val = (int) ((ISingleProperty) submodel.getProperties().get("testProp1")).get();
		property2Val = (int) ((ISingleProperty) submodel.getProperties().get("testProp2")).get();
		...
		// Change values back so that other test cases run again
		// - Change property values (shortcut here, we know that it is a single property type)
		((ISingleProperty) submodel.getProperties().get("testProp1")).set(1);
		((ISingleProperty) submodel.getProperties().get("testProp2")).set(2);
	}
}
