    ...
    namespace opcua
    {
        namespace aas
        {
            ...
            inline UA_StatusCode ApiSubmodel<CONNECTOR_TYPE>::createSubmodel(const Submodel_t & t_sm)
            {
                ...
                std::string idTypeVal = submodel::KeyType_::to_string(t_sm.getIdentification().getIdType());
                ...
                // Write SubmodelKind
                status = setSubmodelKind(t_sm.getIdShort(), basyx::submodel::ModelingKind_::to_string(t_sm.getKind()));
            ...
            inline std::unique_ptr<Submodel_t> ApiSubmodel<CONNECTOR_TYPE>::constructSubmodel(const NodeId & t_smNode)
            {
                ...
                auto sm = util::make_unique<Submodel>(idShort, Identifier_t(submodel::KeyType_::from_string(idTypeVal), idVal));

                sm->setAdministrativeInformation(submodel::map::AdministrativeInformation(admVer, admRev));
            ...
            std::vector<std::unique_ptr<IProperty_t>> ApiSubmodel<CONNECTOR_TYPE>::getAllProperties(const std::string& t_idShort)
            {
                ...
                std::vector<std::unique_ptr<basyx::submodel::api::IProperty>> ret;
